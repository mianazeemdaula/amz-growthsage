    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-y-8 md:gap-x-8 mt-8">
        <div class="relative col-span-2">
            ...
            <div class="p-6 rounded-lg  bg-white">
                ...
                <form action="{{ route('admin.faq-cats.faqs.store', $catId) }}" method='post' class="flex flex-col w-full mt-4" enctype="multipart/form-data">
                    @csrf
                    <div class="grid grid-cols-1 lg:grid-cols-2 mt-3 text-slate-600 gap-4">
                        <div>
                            <label for="">Question*</label>
                            <input type="text" name='question' value="{{ old('question') }}" class='custom-input'>
                            @error('question')
                        ...
                        <div class="lg:col-span-2">
                            ...
                            <input type="text" name='answer' value="{{ old('answer') }}" class='custom-input'>
                            @error('answer')
                            <p class="text-red-500 text-xs italic">{{ $message }}</p>
                            @enderror
                        </div>
