        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-y-8 md:gap-x-8 mt-8">
            <div class="relative col-span-2">
                ...
                <div class="p-6 rounded-lg  bg-white">
                    ...
                    <form action="{{ route('admin.courses.update', $model->id) }}" method='post'
                        ...
                        <div class="grid grid-cols-1 lg:grid-cols-2 mt-3 text-slate-600 gap-4">
                            ...
                            <div>
                                ...
                                <input type="text" name='lectures_count' value="{{ $model->lectures_count }}"
                                    class='custom-input'>
                                @error('fee')
                                    <p class="text-red-500 text-xs italic">{{ $message }}</p>
                                @enderror
                            ...
                            <div>
                                <label for="">Teamleader Reward</label>
                                <input type="text" name='teamleader_reward' value="{{ $model->teamleader_reward }}"
                                    class='custom-input'>
                                @error('teamleader_reward')
                                    <p class="text-red-500 text-xs italic">{{ $message }}</p>
                                @enderror
                            </div>
                            <div>
                                <label for="">Referral Reward</label>
                                <input type="text" name='referral_reward' value="{{ $model->referral_reward }}"
                                    class='custom-input'>
