...
@section('body')
    <div class="container px-6">
        ...
        <!-- pallet boxes -->
        <x-admin.pallets :user='Auth::user()'></x-admin.pallets>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-y-8 md:gap-x-8 mt-8">
            <div class="relative col-span-2">
                ...
                <div class="p-6 rounded-lg  bg-white">
                    ...
                    <form action="{{ route('admin.change_password') }}" method="post" class="mt-8 md:px-24"
                        onsubmit="return validate(event)">
                        @csrf
                        <div class="grid grid-cols-1 gap-4">
                            <div>
                                <label for="" class="text-slate-600 text-sm">Current Password</label>
                                <input type="text" id="current" name="current" class="custom-input"
                                    placeholder="Current password" required>

                            </div>
                            <div>
                                <label for="" class="text-slate-600 text-sm">New Password</label>
                                <input type="password" id="new" name="new" class="custom-input"
                                    placeholder="New password" required>

                            </div>
                            <div>
                                <label for="" class="text-slate-600 text-sm">Confirm Password</label>
                                <input type="password" id="confirm" class="custom-input"
            ...
            <div>
                <div class="p-6 bg-white">
                    <x-admin.profile :user="Auth::user()" mode='full'></x-admin.profile>
