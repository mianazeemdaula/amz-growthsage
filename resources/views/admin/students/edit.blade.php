        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-y-8 md:gap-x-8 mt-8">
            <div class="relative col-span-2">
                ...
                <div class="p-6 rounded-lg  bg-white">
                    ...
                    <form action="{{ route('admin.students.update', $model->id) }}" method='post'
                        ...
                        <div class="grid grid-cols-1 lg:grid-cols-2 mt-3 text-slate-600 gap-4">
                                ...
                                <select name="language_id" id="" class="w-full custom-input">
                                    @foreach ($languages as $language)
                                        <option value="{{ $language->id }}"
                                            @if ($model->profile->lanugage_id == $language->id) selected @endif>{{ $language->name }}</option>
                                    @endforeach
                                </select>
                                ...
                                <select name="country_id" id="" class="w-full custom-input">
                                    @foreach ($countries as $country)
                                        <option value="{{ $country->id }}"
                                            @if ($model->profile->country_id == $country->id) selected @endif>{{ $country->name }}</option>
                                    @endforeach
                                </select>
                                @error('country_id')
                                    <p class="text-red-500 text-xs italic">{{ $message }}</p>
                            ...
                            <div>
                                ...
                                <input type="text" name='province' value="{{ $model->profile->province }}"
                                    class='custom-input'>
                                @error('province')
                                    <p class="text-red-500 text-xs italic">{{ $message }}</p>
                                @enderror
                            ...
                            <div>
                                ...
                                <input type="text" name='city' value="{{ $model->profile->city }}"
                                    class='custom-input'>
                                @error('city')
                                    <p class="text-red-500 text-xs italic">{{ $message }}</p>
                                @enderror
                            ...
                            <div class="lg:col-span-2">
                                ...
                                <input type="text" name='address' value="{{ $model->profile->address }}"
                                    class='custom-input'>
                                @error('address')
                                    <p class="text-red-500 text-xs italic">{{ $message }}</p>
                                @enderror
                            ...
                            <div>
                                ...
                                <input type="text" name='phone' value="{{ $model->profile->phone }}"
                                    class='custom-input'>
                                @error('phone')
                                    <p class="text-red-500 text-xs italic">{{ $message }}</p>
                                @enderror
                            ...
                            <div>
                                ...
                                <input type="text" name='experience' value="{{ $model->profile->experience }}"
                                    class='custom-input'>
                                @error('experience')
                                    <p class="text-red-500 text-xs italic">{{ $message }}</p>
                                @enderror
                            ...
                            <div>
                                ...
                                <input type="text" name="referral_code" value="{{ $model->code }}" class="custom-input"
                                    placeholder="Referral Code if any" readonly>
                            ...
                            <div>
                                ...
                                <select name="role" id="" class="w-full custom-input">
                                    @foreach (['student', 'teamleader'] as $type)
                                        <option value="{{ $type }}"
                                            @if ($model->hasRole($type)) selected @endif>{{ Str::ucfirst($type) }}
                                        </option>
                                    @endforeach
                                </select>
                                @error('country_id')
